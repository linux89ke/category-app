...
Redesigned — Sharp UI + Plotly + Category Explorer
"""
...
.app-header { display: flex; align-items: baseline; gap: 12px; margin-bottom: 0.25rem; }
.app-title { font-family: 'Syne', sans-serif; font-size: 2rem; font-weight: 800; color: #f55036; letter-spacing: -0.03em; line-height: 1; }
.app-subtitle { font-family: 'DM Mono', monospace; font-size: 0.75rem; color: #555; letter-spacing: 0.05em; text-transform: uppercase; }
...
.stat-value { font-family: 'Syne', sans-serif; font-size: 1.6rem; font-weight: 700; color: #f55036; line-height: 1; }
.stat-label { font-size: 0.62rem; color: #444; text-transform: uppercase; letter-spacing: 0.08em; margin-top: 4px; }
...
.result-label { font-size: 0.62rem; color: #444; text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 8px; }
.result-category { font-family: 'Syne', sans-serif; font-size: 1.1rem; font-weight: 600; color: #f0f0f0; word-break: break-word; }

.badge-cache { display: inline-block; background: #0a1a0a; border: 1px solid #1a3a1a; color: #4caf72; font-size: 0.62rem; padding: 2px 8px; border-radius: 20px; letter-spacing: 0.08em; margin-bottom: 8px; }
.badge-ai { display: inline-block; background: #1a0d08; border: 1px solid #3a1a0a; color: #f55036; font-size: 0.62rem; padding: 2px 8px; border-radius: 20px; letter-spacing: 0.08em; margin-bottom: 8px; }

.section-label { font-size: 0.62rem; text-transform: uppercase; letter-spacing: 0.1em; color: #333; margin-bottom: 0.5rem; }

button[data-baseweb="tab"] { font-family: 'DM Mono', monospace !important; font-size: 0.75rem !important; text-transform: uppercase; letter-spacing: 0.08em; }
...
section[data-testid="stSidebar"] label { font-family: 'DM Mono', monospace !important; font-size: 0.68rem !important; text-transform: uppercase; letter-spacing: 0.07em; color: #444 !important; }
...
<div class="app-header">
    ...
    <span class="app-subtitle">/ product category intelligence</span>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    st.markdown('<p style="font-family:Syne,sans-serif;font-weight:700;font-size:0.95rem;color:#f55036;margin-bottom:1rem;letter-spacing:-0.01em;">⚙ CONFIG</p>', unsafe_allow_html=True)
    ...
    if api_key:
        st.markdown('<p style="color:#4caf72;font-size:0.7rem;letter-spacing:0.05em;">● KEY ACTIVE</p>', unsafe_allow_html=True)
    else:
        st.markdown('<p style="color:#f55036;font-size:0.7rem;letter-spacing:0.05em;">● NO KEY — AI DISABLED</p>', unsafe_allow_html=True)
    ...
    st.markdown(f'<p style="font-family:Syne,sans-serif;font-size:1.3rem;font-weight:700;color:#f55036;margin-bottom:0;">{len(cache_data)}</p><p style="font-size:0.62rem;color:#333;text-transform:uppercase;letter-spacing:0.08em;margin-top:2px;">CACHED PREDICTIONS</p>', unsafe_allow_html=True)
...
if not map_file:
    ...
    uploaded_map = st.file_uploader("Upload category file", type=["csv", "xlsx"])
    if uploaded_map:
        df_map = pd.read_excel(uploaded_map) if uploaded_map.name.endswith(".xlsx") else pd.read_csv(uploaded_map)
        df_map.to_csv("category_map1.csv", index=False)
...
leaves, vectorizer, matrix, raw_df, all_paths = load_index(map_file)
...
st.markdown(f"""
<div class="stat-row">
    <div class="stat-card"><div class="stat-value">{len(leaves):,}</div><div class="stat-label">Leaf Categories</div></div>
    <div class="stat-card"><div class="stat-value">{len(top_level_cats):,}</div><div class="stat-label">Top-Level Groups</div></div>
    <div class="stat-card"><div class="stat-value">{depth_max}</div><div class="stat-label">Max Depth</div></div>
    <div class="stat-card"><div class="stat-value">{len(cache_data):,}</div><div class="stat-label">Cached Predictions</div></div>
</div>
""", unsafe_allow_html=True)
...
def render_path(cat_str):
    ...
    for i, part in enumerate(parts):
        if i == len(parts) - 1:
            html.append(f'<span style="color:#f55036">{part}</span>')
        else:
            html.append(f'<span style="color:#555">{part}</span>')
...
with tab1:
    ...
    if predict_btn and query.strip():
        ...
        if cached:
            ...
            st.markdown(f'<div class="result-card"><div class="result-label">Best Match</div><div class="result-category">{render_path(cat_display)}</div></div>', unsafe_allow_html=True)
        ...
        # Candidate bar chart
        st.markdown('<p class="section-label" style="margin-top:1.5rem;">Top Candidates by TF-IDF Similarity</p>', unsafe_allow_html=True)
        display_n = min(15, len(cands))
        short_labels = []
        for c in cands[:display_n]:
            parts = c.split(" / ")
            short_labels.append(parts[-1] + ("  [" + parts[0] + "]" if len(parts) > 1 else ""))

        fig = go.Figure(go.Bar(
            x=scores[:display_n],
            y=short_labels,
            orientation='h',
            marker=dict(color=scores[:display_n], colorscale=[[0,'#130803'],[0.5,'#6a1f0e'],[1,'#f55036']], showscale=False),
            text=[f"{s:.3f}" for s in scores[:display_n]],
...
with tab2:
    ...
    with col_up:
        uploaded = st.file_uploader("CSV / XLSX — column A = product titles", type=["csv", "xlsx"], key="batch")
    with col_info:
        st.markdown("""
        <div style="margin-top:1.5rem;padding:1rem;background:#0a0a0a;border:1px solid #151515;border-radius:8px;">
            <p style="font-size:0.62rem;text-transform:uppercase;letter-spacing:0.1em;color:#333;margin-bottom:6px;">Format</p>
            <p style="font-size:0.8rem;color:#666;">Column A: product titles<br>Other columns ignored<br>CSV or XLSX accepted</p>
        </div>
        """, unsafe_allow_html=True)

    if uploaded:
        df_batch = pd.read_excel(uploaded) if uploaded.name.endswith(".xlsx") else pd.read_csv(uploaded)
        titles = df_batch.iloc[:, 0].astype(str).tolist()
        st.caption(f"{len(titles):,} products loaded from file")
        ...
        if st.button("▶ Run Batch", type="primary"):
            ...
            if todo and api_key:
                ...
                try:
                    ...
                    save_cache(cache)
                    progress.empty()
                    cached_count = len(results) - len(todo)
                    st.success(f"✓ {len(todo)} predicted · {cached_count} from cache · {len(results)} total")
                    ...
                    # Distribution chart
                    st.markdown('<p class="section-label" style="margin-top:1.5rem;">Top-Level Category Distribution</p>', unsafe_allow_html=True)
                    done_rows = [r for r in results if "Done" in str(r.get("Status","")) or "Cached" in str(r.get("Status",""))]
                    if done_rows:
                ...
                progress.empty()
                st.warning("No API key — cached results shown only.")
...
with tab3:
    st.markdown('<p class="section-label">Search & Explore the Category Tree</p>', unsafe_allow_html=True)
    ...
    if search_q.strip():
        sq = search_q.lower()
        filtered = [l for l in leaves if sq in l.lower()]
    if depth_opt != "All":
    ...
    with col_a:
        st.markdown('<p class="section-label">Categories per Top-Level Group</p>', unsafe_allow_html=True)
    ...
    with col_b:
        st.markdown('<p class="section-label">Depth Distribution</p>', unsafe_allow_html=True)
        depths = pd.Series([len(l.split(" / ")) for l in filtered]).value_counts().sort_index()
        fig_dep = go.Figure(go.Bar(
            x=[f"Level {i}" for i in depths.index],
            y=depths.values.tolist(),
    ...
    # Treemap
    st.divider()
    st.markdown('<p class="section-label">Treemap — Click to Drill Down</p>', unsafe_allow_html=True)

    treemap_data = defaultdict(int)
    for leaf in filtered:
        parts = leaf.split(" / ")
        top = parts[0]
        second = parts[1] if len(parts) >= 2 else top
        treemap_data[(top, second)] += 1

    top_totals = defaultdict(int)
    for (top, second), count in treemap_data.items():
        top_totals[top] += count

    tm_ids, tm_labels, tm_parents, tm_values = ["root"], ["All"], [""], [0]

    for top, total in sorted(top_totals.items(), key=lambda x: -x[1])[:35]:
        tm_ids.append(top)
        tm_labels.append(top)
        tm_parents.append("root")
        tm_values.append(total)

    seen_ids = set(tm_ids)
    for (top, second), count in sorted(treemap_data.items(), key=lambda x: -x[1]):
        node_id = f"{top}||{second}"
        if top in seen_ids and node_id not in seen_ids and second != top:
            tm_ids.append(node_id)
            tm_labels.append(second)
            tm_parents.append(top)
            tm_values.append(count)
    ...
    # Raw list
    st.divider()
    st.markdown('<p class="section-label">Raw Category List</p>', unsafe_allow_html=True)

    show_n = st.slider("Rows to show", 10, min(500, max(10, len(filtered))), min(50, len(filtered)), key="ex_rows")
    ...
    if len(filtered) > show_n:
        st.caption(f"Showing {show_n:,} of {len(filtered):,} — use slider to see more")

    st.download_button(
        "↓ Download Filtered Categories",
        pd.DataFrame({"category_path": filtered}).to_csv(index=False).encode(),
        "categories_filtered.csv",
        "text/csv",
    )
